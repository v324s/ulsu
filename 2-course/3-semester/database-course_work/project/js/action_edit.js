function action_edit(table, id) {
    if (confirm("Отредактировать строку " + id + " ?")) {
        let arr = {};
        arr.action = 'edit';
        arr.table = table;
        arr.id = id;
        switch (table) {
            case 'category':
                let category_name = prompt('Новое имя категории:');
            ...
            case 'sizes':
                let sizes_num = prompt('Новый номер размера:');
                arr.number = sizes_num;
                break;

            case 'colors':
                let colors_name = prompt('Новый номер размера:');
                arr.name = colors_name;
                break;
